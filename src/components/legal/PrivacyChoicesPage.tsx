  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (applicableLegislation === PrivacyLegislation.CCPA && requestType === 'optOut') {
        ...
        // Сохраняем email отдельно, если нужно для уведомлений
        if (email) {
          // Здесь можно добавить логику для сохранения email, если это необходимо
          console.log('Saving email for notifications:', email);
        }
      }
